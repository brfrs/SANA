...
void Graph::serializeGraph(Graph& G, string outputName, bool typedNodes, bool locked)
{
    ...
    if (fd != -1)
    {
        ...
        string outName = "networks/" + outputName + "/autogenerated/" + outputName + (typedNodes ? "_TypedNodes" : "_UntypedNodes")
            + (locked ? "_Locked" : "_Unlocked") + (multi_pairwise ? "_Multi_pairwise" : "_Not_multi_pairwise") + ".bin";

        string tmpName = "/tmp/" + outputName + (typedNodes ? "_TypedNodes" : "_UntypedNodes")
...
void Graph::serializeShadow(Graph& G)
{
    ...
    if (fd != -1)
    {
        ...
        string outName = "networks/" + name + "/autogenerated/" + name + "_NodeNameToIndexMap" + ".bin";
...
void Graph::loadFromGraphmlFile(string fin, string graphName, Graph& g, bool nodesHaveTypes) {
...
void Graph::loadFromCsvFile(string fin, string graphName, Graph& g, bool nodesHaveTypes) {
    ifstream infile(fin.c_str());
    string line;
    unordered_set<string> record;
    size_t lineCount = 0;
...
vector<vector<float> > Graph::loadGraphletDegreeVectors(int maxGraphletSize) {
    std::ostringstream oss;
    oss << maxGraphletSize;
    string gdvsFileName = autogenFilesFolder() + name + "_gdv"+oss.str()+".bin";
    uint n = getNumNodes();
    if (fileExists(gdvsFileName)) {
        vector<vector<float> > gdvs;
        if(maxGraphletSize == 4){
            gdvs.assign(n, vector<float>(15));
        }else{//it is either 4 or 5
            gdvs.assign(n, vector<float>(73));
        }
    ...
    Timer T;
    T.start();
    vector<vector<float> > gdvs = computeGraphletDegreeVectors(maxGraphletSize);
...
vector<vector<float> > Graph::computeGraphletDegreeVectors(int maxGraphletSize) {
    std::cout<<"Computing Graphlet Degree Vectors... "<<endl;
    uint n = getNumNodes();
    uint m = getNumEdges();

    string fileName = "tmp/compute_gdvs.txt";
    ...
    vector<vector<float> > gdvs = computeGraphlets(maxGraphletSize, fileName);
    return gdvs;
}
...
void Graph::saveGraphletsAsSigs(string outputFile) {
    std::vector<std::vector<float>> graphlets = computeGraphletDegreeVectors(5); // This function only uses k = 5
    vector<string> nodeNames = getNodeNames();
    ofstream out;
