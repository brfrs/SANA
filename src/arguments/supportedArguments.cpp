...
vector<array<string, 6>> supportedArguments = {
  ...
  { "-localScoresFile", "string", "sana", "Local Scores File basename", "Specifies the basename of the local score file; actual output files will append (.out, .align, etc) to this.", "0"},
    ...
    { "-approxGraphletOrbits", "bool", "false", "Use BLANT to approximate GDVs", "GDVs are approximated using BLANT for Graphlet-based objective function.", "0"},
    { "-blantArgsG1", "string", "", "BLANT arguments for G1", "Arguments to run BLANT with on graph G1. Used when -approxGraphletOrbits is true.", "0" },
    { "-blantArgsG2", "string", "", "BLANT arguments for G2", "Arguments to run BLANT with on graph G2. Used when -approxGraphletOrbits is true.", "0" },
